...
export default function NewCarsPage() {
  ...
  const selectMake = async (make: Make) => {
    ...
    try {
      setLoading(true);
      const allModels = await getPublicModels(make.id);
      setModels(allModels);
      setViewStep("models");
    } catch (e) {
  ...
  const selectModel = async (model: CarModel) => {
    ...
    try {
      setLoading(true);
      const allVersions = await getPublicVersions(model.id);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br mt-16 from-blue-50 to-indigo-50">
